...
[McpServerToolType]
public static class EraseMetadataTool
{
    [McpServerTool, Description(
        "Erases metadata fields from a document (author, title, company, keywords, dates, etc.) and saves the cleaned file to storage. " +
        ...
        "On failure, the response text starts with 'Metadata erasure failed for' followed by the underlying exception type, message, and inner-exception chain.")]
    public static async Task<string> EraseMetadata(
        ...
        [Description("Password for protected documents")] string? password = null)
    {
        ...
        try
        {
            ...
            return prefix + await output.BuildFileOutputAsync(savedPath, $"Metadata erased (fields: {fields}, status: {result.Status})");
        }
        catch (Exception ex)
        {
