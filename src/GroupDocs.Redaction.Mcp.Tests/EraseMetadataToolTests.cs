...
public class EraseMetadataToolTests
{
    ...
    public EraseMetadataToolTests()
    {
        _output = new OutputHelper(_storage.Object, Microsoft.Extensions.Options.Options.Create(new McpConfig()));
    }

    [Fact]
    public async Task EraseMetadata_WhenResolverThrows_PropagatesException()
    {
        _resolver
        ...
        await Assert.ThrowsAsync<FileNotFoundException>(() =>
            EraseMetadataTool.EraseMetadata(
                _resolver.Object, _storage.Object, _licenseManager.Object, _output,
                new FileInput { FilePath = "missing.pdf" }));
    }

    [Fact]
    public async Task EraseMetadata_WhenResolverThrows_DoesNotWriteToStorage()
    {
        _resolver
            .Setup(r => r.ResolveAsync(It.IsAny<FileInput>(), It.IsAny<CancellationToken>()))
            .ThrowsAsync(new FileNotFoundException("missing.pdf"));
    ...
    [Fact]
    public async Task EraseMetadata_SetsLicense_BeforeResolving()
    {
    ...
    [Fact]
    public async Task EraseMetadata_PassesFileInputToResolver_Unchanged()
    {
