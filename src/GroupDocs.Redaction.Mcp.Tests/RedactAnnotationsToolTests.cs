...
public class RedactAnnotationsToolTests
{
    ...
    public RedactAnnotationsToolTests()
    {
        _output = new OutputHelper(_storage.Object, Microsoft.Extensions.Options.Options.Create(new McpConfig()));
    }

    [Fact]
    public async Task RedactAnnotations_WhenResolverThrows_PropagatesException()
    {
        _resolver
        ...
        await Assert.ThrowsAsync<FileNotFoundException>(() =>
            RedactAnnotationsTool.RedactAnnotations(
                _resolver.Object, _storage.Object, _licenseManager.Object, _output,
                new FileInput { FilePath = "missing.pdf" }));
    }

    [Fact]
    public async Task RedactAnnotations_WhenResolverThrows_DoesNotWriteToStorage()
    {
        _resolver
            .Setup(r => r.ResolveAsync(It.IsAny<FileInput>(), It.IsAny<CancellationToken>()))
            .ThrowsAsync(new FileNotFoundException("missing.pdf"));
    ...
    [Fact]
    public async Task RedactAnnotations_SetsLicense_BeforeResolving()
    {
    ...
    [Fact]
    public async Task RedactAnnotations_PassesFileInputToResolver_Unchanged()
    {
